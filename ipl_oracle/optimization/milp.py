...
If the equality `team_size == n` is infeasible (typical when a curated
squad has too few domestic players to satisfy the overseas cap), the
solver retries with `team_size ≤ n` and tags the result as relaxed.
"""
...
class SelectionMILP:
    ...
    def solve(
        ...
        result = self._solve_once(
            squad, weights, must_include, must_exclude, team_size_eq=True
        )
        if result.status != "Optimal":
            relaxed = self._solve_once(
                squad, weights, must_include, must_exclude, team_size_eq=False
            )
            if relaxed.status == "Optimal":
                relaxed.relaxed = True
                relaxed.relaxation_reason = (
                    f"squad cannot field {cfg.n_players} under role/overseas caps "
                    f"({len(relaxed.selected)} selected)"
                )
                result = relaxed
    ...
    def _solve_once(
        ...
        team_total = pulp.lpSum(x.values())
        if team_size_eq:
            prob += team_total == cfg.n_players, "team_size"
        else:
            prob += team_total <= cfg.n_players, "team_size_max"
            prob += team_total >= 1, "team_size_min"
        ...
        if wk:
            wk_sum = pulp.lpSum(x[p.player_id] for p in wk)
            prob += wk_sum >= cfg.min_wicketkeepers, "min_wk"
            prob += wk_sum <= cfg.max_wicketkeepers, "max_wk"
        if bowlers:
        ...
        return MILPResult(
            selected=selected,
            excluded=excluded if status == "Optimal" else list(squad),
            objective_value=float(pulp.value(prob.objective) or 0.0) if status == "Optimal" else 0.0,
            solve_time_ms=0.0,
            status=status,
        )
